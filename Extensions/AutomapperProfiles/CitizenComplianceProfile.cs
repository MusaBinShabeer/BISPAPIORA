    ...
    public class CitizenComplianceProfile : Profile
    {
        ...
        public CitizenComplianceProfile()
        {
            CreateMap<AddCitizenComplianceDTO, tbl_citizen_compliance>()
             ...
             .ForMember(d => d.fk_citizen_scheme, opt => opt.MapFrom(src => Guid.Parse(src.fkCitizenScheme)));
            CreateMap<UpdateCitizenComplianceDTO, tbl_citizen_compliance>()
             ...
             .ForMember(d => d.fk_citizen_scheme, opt => opt.MapFrom((src, dest) => otherServices.Check(src.fkCitizenScheme) ? Guid.Parse(src.fkCitizenScheme) : dest.fk_citizen_scheme));
            ...
            //.ForMember(d => d.citizenComplianceStartingMonth, opt => opt.MapFrom((src) => src.tbl_citizen_scheme.citizen_scheme_starting_month))
            //.ForMember(d => d.citizenComplianceCommittedSavingAmount, opt => opt.MapFrom((src) => src.tbl_citizen_scheme.citizen_scheme_saving_amount))
            .ForMember(d => d.fkCitizen, opt => opt.MapFrom((src) => src.fk_citizen))
            .ForMember(d => d.fkCitizenScheme, opt => opt.MapFrom((src) => src.fk_citizen_scheme));
        }
    }
}
